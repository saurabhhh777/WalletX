 ...
 return (
  <div className="bg-white border border-gray-200 rounded-xl p-6 mt-8">
   ...
   <div className="flex items-center justify-between mb-4">
    <h3 className="text-lg font-bold text-black font-jost">Solana History</h3>
    <button onClick={load} disabled={loading} className="text-sm bg-gray-100 text-black px-3 py-1 rounded-md hover:bg-gray-200 transition-colors">
      ...
      <tbody>
       {items.map((it) => (
        <tr key={it.signature} className="border-t border-gray-100">
         <td className="py-2 pr-4 capitalize text-black">{it.type}</td>
         <td className="py-2 pr-4 text-black">{it.amount}</td>
         <td className="py-2 pr-4 truncate max-w-[220px] text-black">{it.from || '-'}</td>
         <td className="py-2 pr-4 truncate max-w-[220px] text-black">{it.to || '-'}</td>
         <td className="py-2 pr-4 text-black">{it.timestamp ? new Date(it.timestamp * 1000).toLocaleString() : '-'}</td>
